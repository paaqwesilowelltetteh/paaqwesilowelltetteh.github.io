    ...
    <div class="container max-w-screen-xl mx-auto px-4">
      <div class="flex flex-col lg:flex-row justify-between">
        ...
        <div class="space-y-24">
          <div v-for="(project, index) in projects" :key="index" class="flex space-x-6">
            <h1 class="font-normal text-gray-700 text-3xl md:text-4xl">{{ index + 1 }}</h1>
            <span class="w-28 h-0.5 bg-gray-300 mt-5"></span>
            <div>
              <h1 class="font-normal text-gray-700 text-3xl md:text-4xl mb-5">{{ project.title }}</h1>
              <p class="font-normal text-gray-500 text-sm md:text-base" v-html="project.description"></p>
            </div>
          </div>
        </div>
      </div>
